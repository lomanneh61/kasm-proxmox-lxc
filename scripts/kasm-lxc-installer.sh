...
pct create "$CTID" local:vztmpl/$TEMPLATE \
  ...
  --net0 name=eth0,bridge=vmbr0,firewall=0,type=veth \
...
# Create firewall file if missing
if [ ! -f "$FW_FILE" ]; then 
  touch "$FW_FILE" 
fi 

# Allow DHCP (UDP 67/68) and DNS (UDP/TCP 53) 
cat <<EOF >> "$FW_FILE" 
[OPTIONS] 
enable: 1 
[RULES] 
IN ACCEPT -p udp --dport 67 
IN ACCEPT -p udp --dport 68 
